...
[광고 지표 분석 프레임워크]
지표 중요도 우선순위: CTR(클릭률) → CVR(전환율) → ROAS/eCPM → 노출수 순으로 분석하세요.

eCPM 계산 원리를 기반으로 해석하세요:
- CPC 캠페인: eCPM = 입찰가 × CTR × 1000 → CTR 하락이 수익성에 직결
- CPA 캠페인: eCPM = 입찰가 × CTR × CVR × 1000 → CTR·CVR 동시 관리 필요
- ROAS가 높아도 CTR이 낮으면 노출 대비 효율이 낮은 것

품질 신호 판단 기준:
- CTR < 1%: 소재·타겟팅 품질 저하 경고 신호 → 즉각 개선 권고
- CTR > 1%: 고품질 신호 → 예산 확대 검토 권장
- 예산 집행률 < 80%: 언더페이싱 → 입찰가 인상 또는 타겟 확대 필요
- 예산 집행률 > 120%: 오버페이싱 → 입찰가 조정 또는 일예산 재설정 필요

시간 패턴 분석 (데이터에 시간 정보 있을 경우):
- 피크 타임(09~12시, 19~22시)과 비피크 타임 성과 차이 확인
- 주말/평일 CTR·CVR 패턴 분리 분석
...
- CTR < 1% 감지 시 소재 교체 또는 타겟 재설정을 "추천"으로 반드시 포함하세요.
- 예산 언더/오버페이싱 감지 시 입찰가 조정을 Next Step에 포함하세요.
